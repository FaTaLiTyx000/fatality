...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Farhan\n'
            + 'ORG:Creator FXC7;\n'
            + 'TEL;type=CELL;type=VOICE;waid=557398607710:+55 73 9860-7710\n'
            + 'END:VCARD'

prefix = "_"
name = "~ 🔥🇹🇷࿗ Fz'K𝜦͢ 𝑰 KE-𝚋𝚘𝚝"
rdaftar = "OBRIGADO POR SE REGISTRAR😁"
rmenu = "Olá! amigos do K𝜦͢ 𝑰 KE-𝚋𝚘𝚝👋"
limitt = 50
ban = []
userpremium = ["557398607710@s.whatsapp.net"] //ubah nomer kalian
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau e não volte @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
...
const getRegisteredRandomId = () => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans = ["557398607710@s.whatsapp.net"] // apenas mude esta tudo bem
			...
			mess = {
				...
				only: {
					...
					premium: '*𝗗𝗘𝗦𝗖𝗨𝗟𝗣𝗘 𝗘𝗦𝗧𝗘 𝗥𝗘𝗖𝗨𝗥𝗦𝗢 𝗣𝗔𝗥𝗔 𝗨𝗦𝗨A𝗥𝗜𝗢 𝗣??𝗘𝗠𝗜𝗨𝗠 𝗘𝗦𝗣𝗘𝗖I𝗙I𝗖𝗢!!*',
			...
			const ownerNumber = ["557398607710@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```Lista dos Banidos``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```Lista Users Premium  K𝜦͢ 𝑰 KE-𝚋𝚘𝚝``` :\n'
